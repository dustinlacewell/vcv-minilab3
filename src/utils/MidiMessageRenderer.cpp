...
std::string MidiMessageRenderer::renderNoteOn(midi::Message &msg, std::string chan, std::string note) {
    uint8_t vel = msg.getValue();
    std::string s = string::f("%s %s %s | %i", chan.c_str(), pads(5, "ON").c_str(), note.c_str(), vel);
    return s;
}

std::string MidiMessageRenderer::renderNoteOff(midi::Message &msg, std::string chan, std::string note) {
    uint8_t vel = msg.getValue();
    std::string s = string::f("%s %s %s | %i", chan.c_str(), pads(5, "OFF").c_str(), note.c_str(), vel);
    return s;
}

std::string MidiMessageRenderer::renderKeyPressure(midi::Message &msg, std::string chan, std::string note) {
    uint8_t value = msg.getValue();
    std::string s = string::f("%s %s %s | %i", chan.c_str(), pads(5, "TOUCH").c_str(), note.c_str(), value);
    return s;
}

std::string MidiMessageRenderer::renderCC(midi::Message &msg, std::string chan, std::string note) {
    int8_t value = msg.bytes[2];
    std::string s = string::f("%s %s %s | %i", chan.c_str(), pads(5, "CC").c_str(), note.c_str(), value);
    return s;
}

std::string MidiMessageRenderer::renderProgramChange(midi::Message &msg, std::string chan) {
    uint8_t prog = msg.getNote();
    std::string s = string::f("%s %s %i", chan.c_str(), pads(5, "PROG").c_str(), prog);
    return s;
}

std::string MidiMessageRenderer::renderChannelPressure(midi::Message &msg, std::string chan) {
    uint8_t value = msg.getNote();
    std::string s = string::f("%s PRESS %i", chan.c_str(), value);
    return s;
}

std::string MidiMessageRenderer::renderPitchWheel(midi::Message &msg, std::string chan) {
    uint8_t value = msg.getValue();
...
std::string MidiMessageRenderer::renderSystemMessage(midi::Message &msg) {
    switch (msg.getChannel()) {
        ...
        case 0xc: // stop
            ...
            break;
        default: return "UNKOWN SYSTEM MSG";
            break;
    }
}
