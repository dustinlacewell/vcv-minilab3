...
template <typename T>
void ExpanderRelay<T>::onExpanderChange(
    const Module::ExpanderChangeEvent& e, Module::Expander leftExpander,
    Module::Expander rightExpander
) {
    if (e.side != side) {
        return;
    }
