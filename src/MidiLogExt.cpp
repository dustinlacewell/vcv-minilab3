...
struct MidiLogExt : Module {
    ...
    MidiLogExt() {
        ...
        midiCallback = new RelayCallback<midi::Message&>(
            std::bind(&MidiLogExt::processMessage, this, std::placeholders::_1)
        );
        statusCallback = new RelayCallback<bool>(
            std::bind(&MidiLogExt::adjustLight, this, std::placeholders::_1)
        );

        connectedCallback = new RelayCallback<MiniLab3*>(std::bind(
            &MidiLogExt::expanderConnected, this, std::placeholders::_1
        ));
        disconnectedCallback = new RelayCallback<MiniLab3*>(std::bind(
            &MidiLogExt::expanderDisconnected, this, std::placeholders::_1
        ));
        ...
        disconnectedCallback->registerCallback(
            &(expanderRelay->disconnectedRelay)
        );
...
struct MidiLogExtWidget : ModuleWidget {
    ...
    MidiLogExtWidget(MidiLogExt* module) {
        setModule(module);
        ...
        addChild(createLight<SmallLight<GreenLight>>(
            mm2px(Vec(31.233, 7.894)), module, MidiLogExt::CONNECTED_LIGHT
        ));
...
Model* modelMidiLogExt =
    createModel<MidiLogExt, MidiLogExtWidget>("MidiLogExt");