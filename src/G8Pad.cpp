...
void G8Pad::processMidi(midi::Message& msg) {
...
void G8Pad::processPitchBend(midi::Message& msg) {
	bend->setTarget(((uint16_t)msg.getValue() << 7) | msg.getNote());
}

void G8Pad::processAftertouch(midi::Message& msg) {
	int note = msg.getValue();
	touch->setTarget(note);
}

void G8Pad::processKnob(int knob, int value) {
	if (!gateOpen) {
		return;
	}
	if (value == 62) {
...
void G8Pad::processControlChange(midi::Message& msg) {
