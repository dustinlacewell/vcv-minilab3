...
struct MidiLog : Module {
    ...
    void onReset() override {
        ...
        midiLogMessages.push(
            string::f("RATE %i", int(APP->engine->getSampleRate()))
        );
        Module::onReset();
    }
        ...
        switch (msg.getStatus()) {
            case 0x9:  // note on
                if (!midiLogMessages.full() && showNoteMsg) {
                    ...
                    std::string s = string::f(
                        "%s %s %s | %i", chan.c_str(), pads(5, "ON").c_str(),
                        note.c_str(), vel
                    );
            ...
            case 0x8:  // note off
                if (!midiLogMessages.full() && showNoteMsg) {
                    uint8_t vel = msg.getValue();
                    std::string s = string::f(
                        "%s %s %s | %i", chan.c_str(), pads(5, "OFF").c_str(),
                        note.c_str(), vel
                    );
                    midiLogMessages.push(s);
                }
            ...
            case 0xa:  // key pressure
                if (!midiLogMessages.full() && showKeyPressure) {
                    ...
                    std::string s = string::f(
                        "%s %s %s | %i", chan.c_str(), pads(5, "PRESS").c_str(),
                        note.c_str(), value
                    );
                    midiLogMessages.push(s);
                }
            ...
            case 0xb:  // cc
                if (!midiLogMessages.full() && showCcMsg) {
                    ...
                    std::string s = string::f(
                        "%s %s %s | %i", chan.c_str(), pads(5, "CC").c_str(),
                        note.c_str(), value
                    );
                    midiLogMessages.push(s);
                }
            ...
            case 0xc:  // program change
                if (!midiLogMessages.full() && showProgChangeMsg) {
                    ...
                    std::string s = string::f(
                        "%s %s %i", chan.c_str(), pads(5, "PROG").c_str(), prog
                    );
                    midiLogMessages.push(s);
                }
                break;
            case 0xd:  // channel pressure
                if (!midiLogMessages.full() && showChannelPressurelMsg) {
                    ...
                    std::string s =
                        string::f("%s PRESS %i", chan.c_str(), value);
                    midiLogMessages.push(s);
                }
                break;
            case 0xe:  // pitch wheel
                if (!midiLogMessages.full() && showPitchWheelMsg) {
                    uint16_t value =
                        ((uint16_t)msg.getValue() << 7) | msg.getNote();
                    std::string s =
                        string::f("%s PITCH %i", chan.c_str(), value);
                    midiLogMessages.push(s);
                }
                break;
            case 0xf:  // system
...
struct MidiLogWidget : ModuleWidget {
    ...
    MidiLogWidget(MidiLog* module) {
        ...
        MidiDisplay* display =
            createWidget<MidiDisplay>(mm2px(Vec(0.0, controlsY)));
